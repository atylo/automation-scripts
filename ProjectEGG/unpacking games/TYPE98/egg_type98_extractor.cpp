...
// Helper: allow writing from raw pointer
...
// Overload for vector convenience (wraps the above)
...
void Decrypt(uint8_t* ptr, size_t len, uint8_t seed, const std::vector<uint8_t>& password, int version) {
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        // Calculate the next shift based on the value *after* Layer 1
        ...
        // Capture the current state of 'val' (which is the input to the next chain step)
...
void ExtractFD(const std::vector<uint8_t>& data, const std::string& folder) {
    ...
    while (offset + 0x2B1 < total_len) {
        ...
        // We can read directly from the pointer using offset
