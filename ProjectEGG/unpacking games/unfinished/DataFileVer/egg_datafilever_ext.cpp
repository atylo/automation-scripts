...
// Port of Python's parse_blob_header: walk backward from blob_offset past
// zero padding to find the key byte, then figure out where decomp starts.
// This is NOT a fixed offset — the padding between the key and the blob
// varies, which is why the fixed data[blob_offset-4]/data[blob_offset-8]
// approach silently misreads key/decomp on some files.
struct BlobHeader {
    bool valid;
    uint32_t decomp;
    uint8_t key;
...
BlobHeader parse_blob_header(const std::vector<uint8_t>& data, size_t blob_offset) {
    BlobHeader result{false, 0, 0};

    if (blob_offset < 1) return result;

    size_t pos = blob_offset - 1;
    size_t min_pos = std::max<size_t>(4, blob_offset >= 16 ? blob_offset - 16 : 0);

    while (pos > min_pos && data[pos] == 0x00) {
        pos--;
    }

    if (data[pos] == 0x00 || pos < 4) return result;
    ...
    if (data[P - 1] != 0x00) {
        // Zero key case: P landed on decomp[2] (3rd byte of decomp) because
        // all zeros (key + padding) were skipped.
        key = 0x00;
        ...
        key = data[P];
        if (P < 4) return result;
        decomp_pos = P - 4;
    }

    if (decomp_pos + 4 > data.size()) return result;

    // Verify decomp MSB is 0x00
    if (data[decomp_pos + 3] != 0x00) return result;

    uint32_t decomp = read_u32(data, decomp_pos);
    if (!(decomp > 10000 && decomp < 5000000)) return result;
...
std::vector<DiskEntry> find_type234_metadata(const std::vector<uint8_t>& data, uint32_t image_base,
    ...
    // Process found values into disk entries — bounds match the Python
    // reference (10000 < comp_size < 5000000), not the looser range that
    // let spurious MOV hits through.
    size_t i = 0;
    while (i + 1 < found_values.size()) {
        uint32_t blob_va = found_values[i];
        uint32_t comp_size = found_values[i+1];

        if (is_valid_va(blob_va, sections) && comp_size > 10000 && comp_size < 5000000) {
            uint32_t blob_offset = va_to_offset(blob_va, sections);

            if (blob_offset) {
                BlobHeader hdr = parse_blob_header(data, blob_offset);
                if (!hdr.valid) {
                    std::cout << "Found possible blob at 0x" << std::hex << blob_offset
                              << " and comp 0x" << comp_size << std::dec
                              << ", but no actual blob header" << std::endl;
                    i++;
                    continue;
                }

                DiskEntry entry;
                entry.decomp = hdr.decomp;
                entry.va = blob_va;
...
// Matches the Python reference: names have been observed running longer
// than 16 bytes into what the format calls "reserved" space, so the
// comment-terminator/reserved-bytes/media-type checks are unreliable and
// are not used to reject a blob. Only disk_size and first_track (the two
// fields that have held up in practice) are checked.
bool validate_d88(const std::vector<uint8_t>& data, std::string& name) {
    if (data.size() < 0x2B0) return false;

    // Name field: up to 25 bytes, trailing nulls stripped. Not decoded as
    // Shift-JIS here (no conversion library available in this build) — for
    // non-ASCII names this will contain raw bytes rather than a clean
    // string, same information Python gets before its shift-jis fallback.
    size_t name_len = std::min<size_t>(25, data.size());
    size_t end = 0;
    for (size_t i = 0; i < name_len; i++) {
        if (data[i] == 0x00) break;
        end = i + 1;
    }
    name.assign(reinterpret_cast<const char*>(data.data()), end);
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "EGG DataFileVer extractor version 2" << std::endl;
