...
def extract_archive(file_path):
    ...
    with open(file_path, "rb") as f:
        f.seek(40)
        table_data = f.read(464) # 29 files * 16 bytes
        
        DATA_OFFSET = 850

        print(f"--- Extracting 29 files to '{out_dir}' ---\n")
        ...
        for i in range(29):
            ...
            a1 = i * 4
            meta_offset = descramble_dword0(a1, d0)
            file_offset = descramble_dword1(a1 + 1, d1)
            file_size   = descramble_dword2(a1 + 2, d2)

            # Seek to the absolute data position
            absolute_pos = DATA_OFFSET + file_offset
            f.seek(absolute_pos)
            ...
            # Save it to disk
            out_name = os.path.join(out_dir, f"file_{i:02d}.bin")
            with open(out_name, "wb") as out_f:
                out_f.write(file_data)
                
            print(f"Extracted {out_name} (Size: {file_size} bytes, Offset: {absolute_pos})")

# Run the extractor
extract_archive("ENCS0001.bnd")